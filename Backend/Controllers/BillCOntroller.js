...
exports.getBillByMonthJson = catchAsync(async (req, res) => {
  const { startBodyDate, endBodyDate, isGST } = req.body;
...
async function getBillNo() {
  ...
  let id =
    process.env.TYPE === "PRODUCTION"
      ? "65a8bfafdfb9d72bd46f3a6f"
      : "65a6a1efa5c99e42c492b67c";
...
exports.deleteBill = async (req, res) => {
  try {
    ...
    const deletedBillNo = billToDelete.billNo;
    ...
    await Bill.updateMany(
      { billNo: { $gt: deletedBillNo } },
      { $inc: { billNo: -1 } }
    );

    const lastBill = await Bill.findOne().sort({ billNo: -1 });
    const newBillNo = lastBill ? lastBill.billNo + 1 : 1;
    await IncreaseBillNumber(newBillNo);

    res
      .status(200)
      .json({ success: true, message: "Bill deleted and counter updated" });
  } catch (err) {
    console.error(err);
