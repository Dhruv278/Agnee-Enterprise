...
export const createBill=(billData)=>async(dispatch)=>{
    try{
        ...
        const { data }=await axios.post(`http://localhost:4000/api/v1/submitInvoiceData`,billData,config);
        console.log(data);
        dispatch({
...
export const getBillData=(id)=>async(dispatch)=>{
    try{
        ...
        const { data }=await axios.get(`http://localhost:4000/api/v1/getBillData/${id}`);
        console.log(data);
        dispatch({
